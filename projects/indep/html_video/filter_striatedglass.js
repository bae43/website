...
function draw(v, c, bc, w, h) {
	...
	function swap(arr, cuts, x1, y1, x2, y2) {
		...
		var xdims = w / cuts;
		var ydims = h / cuts;
		for (var i = 0; i < xdims; i += 1) {

			for (var j = 0; j < ydims; j += 1) {

				for (var k = 0; k < 3; k += 1) {

					var cur = data[4*((i + x1) * xdims + (j + y1) * ydims) + k];
					var other = data[4*((i + x2) * xdims + (j + y2) * ydims) + k];
					var temp = cur;
					data[4*((i + x1) * xdims + (j + y1) * ydims) + k] = other;
					data[4*((i + x2) * xdims + (j + y2) * ydims) + k] = temp;
